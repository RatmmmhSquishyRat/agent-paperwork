...
### S-SEND-18 --author 空值拒绝（validation，rework 补录 Pete N2-①）
...
### S-SEND-19 缺 PATH（usage，rework 补录 Pete N2-③）
...
### S-EDIT-09 仅 --stdin 成功（rework 补录 Pete N2-②）
...
### S-READ-09 read --author 习惯迁移（usage，rework 补录 Pete N2-④）
...
### S-OUT-06 --json 与 --plain 同给（usage，rework 补录 Pete N2-⑤）
...
- **When** 检查 `--help` 输出
- **Then** 组/动词集合精确等于 {profile,post,brief,contacts,validate}（隐藏别名不出现）；全 CLI flag 名集合与 spec §4 全表一致；短形式集合精确等于 {-a, -m, -q}（rework 裁定 F3）；`--mention`、`--seq`、`--reply-to`、`--title`、`--participants`、`--to`、`--name`、`--model`、`--entry`、`--entry-title`、`--profile`、`--owner`、`--note`、`--regex`、`--from`、`--limit`、`--type`、`--stdin`、`--json`、`--plain` 逐一断言无短形式（F3 收窄后本白名单断言重获完整防线意义）。
