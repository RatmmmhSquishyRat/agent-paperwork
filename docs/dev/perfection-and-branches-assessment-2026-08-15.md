...
- 更正一：worktree `agent-paperwork-wt-v05perfection` 及其 wip 分支 `wip/v0.5-perfection-snapshot-2026-08-15` 属**另一 agent 的在途工作**，不在本 agent 职责范围。本报告正文凡涉该分支处置的表述（含第四节第 2 条「建议合并轮清理时尽早推送该分支至 origin」、第五节 P-9 末句「该分支在全部成果回流前不得删除」、第三节「wip 快照分支作为恢复源保留」等）一律改读为：该分支的删除/保留/推送均不属本工作流职责范围，不做处置。
- 更正二：owner 从未指示发布 0.6，本报告不涉及发布轮表述；如后续引用本报告作发布轮依据，以「发布时机待 owner 指示，本工作流无发布计划」为准。
- 权威更正口径见 docs/dev/open-items-ledger-2026-08-15.md 第十二节。
