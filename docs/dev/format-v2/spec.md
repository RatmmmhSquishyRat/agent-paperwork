...
- `post send` 保留 `--title`（缺省算法：**剥 `.post.md` 后缀，否则剥 `.md` 后缀，否则取原名**）；**`--to`/`--participants` flag 删除**（owner 追裁 D1/D2）；`--reply-to`/`--mention` 糖衣 flag 的去留见 §11 OQ-4。**仅当文件在锁内为空（size = 0）时**，先写入 preamble（仅 H1 title，D1），再追加消息；文件非空时 `--title` 忽略（见 §11 OQ-1）。
