...
| 问题 | 处置 | 落点 |
|---|---|---|
| C-1（--message hyphen 值缺 allow_hyphen_values 指令） | 按编排层裁定 F4 处理 | spec §3.1 `--message` 行补 clap 属性 `allow_hyphen_values = true` 硬性指令；design §6 补属性前提与副作用边界（`--message --stdin` 连写吞值属显式输入不另设护栏）；impl_plan 步骤(2) 显式列入属性指令；tdd §4 S-SEND-10 用例注明属性依赖（用例注释钉住属性名）；bdd S-SEND-10 断言维持（直传无需 -- 边界、exit 0）；其余 flag 不设的复核结论（--author 不设：防止裸 flag 误写被静默吞为署名值、保护 S-SEND-11 教学防线；--seq u64 类型自防）载入 spec §3.1 与 impl_plan 步骤(2) |
| M-1（缺省判定层级归属错误） | 按编排层裁定 F2 处理 | design §6 与 impl_plan 步骤(2) 改为 clap 层 `required_unless_present = "stdin"` 与 `conflicts_with` 组合（四形态：同给 ArgumentConflict / 皆缺 MissingRequiredArgument 均落 try_parse 失败分支 usage exit 2），命令层无需任何管道；与 spec §3/§5 口径对齐；v0.6_feedbacks §2.3 同步修正并附裁定补记 |
| M-2（二选一 example 断言与 F2/F7 冲突） | 按编排层裁定 F5 处理 | bdd 四处断言（S-SEND-06/07、S-EDIT-04/05）改为「example 为单一静态规范可执行示例（采 --message 通道形态）；二选一指引由 message/fix 文案承担」；design §2.1、tdd §4 同步措辞；spec §5 第 2 条维持单一规范示例口径；回归 v0.5 F2/F7 裁定 |
| M-3（断言语义翻转点与教学文案遗漏） | 已修复（含勘误：见下方注记） | tdd 新增 §1b 断言语义翻转点处置表；impl_plan 步骤(3) 显式列入 usage_fix base 文案与旧 flag 教学清单两个重写点；impl_plan 步骤(4) 引用 tdd §1b；spec §8 迁移链表述修正为「仍为 v0.6 无效 flag（--from 作身份等）的教学链自然延伸」。**勘误（闭合复核 NF-1 定点修复，2026-08-09 如实记录）**：本行初版声称「§1b 含 L457/L501/L987/L1019/L1224/L1295 六处，行号以合并后基线盘点实测校正，含 L1224 负向断言翻转与 L1244 块处置口径」系未实测填报的失实销账：经闭合复核员对照 worktree（HEAD 70f7e43）逐行实测，六行行号与语义映射无一吻合，且遗漏 L457/L501/L1224/L1244 四个核心翻转点；「合并后基线」不存在（impl_plan 步骤(0) 为未来前置步骤）。本轮已按 worktree 逐行实测全量重做 §1b（含四个遗漏翻转点处置行与 main.rs 文案点位实测清单），表头口径更正为「行号基线为 cli-ux-v0.5 worktree HEAD 70f7e43 实测，实施时基线变化须重新盘点」；详见闭合复核报告「定点修复回应」段 |
| M-4（core fix 文案越出步骤(1) 范围） | 已修复 | impl_plan 步骤(1) 补 fix 文案专项：点名 manifest.rs L79/L150/L193 三处（评审时基线行号，实施时以盘点实测校正）改 `--title "My Brief"` 形态，并给出二次盘点命令兑底 |
| m-1（短形式表缺 read --reply-to） | 已修复 | spec §4 「其余无短形式」行补 post read `--reply-to`（无短形式，理由：read 过滤低频）；bdd S-SHORT-02 负向断言清单同步含 `--reply-to`；随裁定 F3 收窄，send 侧 `--reply-to` 亦收回 `-r` 短形式，不对称问题随之消亡 |
| m-2（§4 首行措辞与表格矛盾） | 已随裁定 F3 消除 | 短形式收窄为 {-a, -m, -q} 后全表无跨命令多义，「全 CLI 短形式语义无冲突」表述自然成立，无需降级措辞 |
| m-3（盘点命令口径与 14 处不一致） | 已修复 | impl_plan 步骤(0) 补口径注明（14 处为「需改写文案」口径，非全量命中数；命中行中属注释/断言/非 example 字符串的点位不计入）；tdd §1 末补盘点输出过滤规则（命中行甄别三类：参数层改写 / 语义翻转甄别 / 冻结保留） |
| m-4（文件头文法注释未入刷新范围） | 已修复 | impl_plan 步骤(1) 补「文件头文法注释刷新」条款（core 与 cli 各源文件头部文法描述性注释随同刷新） |

遗留未决项：0。全部修正均为文档层变更，未触碰代码/CI/CHANGELOG。行号口径勘误（闭合复核 NF-1 定点修复）：本评审引用的基线行号（L457/L501/L79 等）为评审时 worktree 实测值，经复核属实；但 rework 轮将其写入 tdd §1b 时未经实测校正即填报，已按 cli-ux-v0.5 worktree（HEAD 70f7e43）逐行实测全量重做；「合并后基线盘点校正」口径失实，更正为「行号基线为 cli-ux-v0.5 worktree HEAD 70f7e43 实测，实施时基线变化须重新盘点」（已写入 tdd §0 勘误注记与 §1b 表头）。
