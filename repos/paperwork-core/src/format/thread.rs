...
// ============================================================================
// Header-regex family — centralized reference point (P-3). The family has
// THREE variants today; their whitespace stances differ ON PURPOSE and must
// not be unified blindly:
//
...
//    scan (spec §5.5) re-checks candidates with `header_seq` directly
//    (P-3 dropped the historical `SEQ_RE` prefilter: it was redundant,
//    `header_seq` is the single authoritative gate, and the tail-scan
//    candidate lines are few by construction).
...
/// definition of the pattern (P-3: `ops/thread.rs` imports it; the ops-side
/// twin was deleted).
///
...
///
/// P-3: dedup runs through the shared [`dedup_preserve_order`]
...
///
/// P-3: delegates to the shared scanner family ([`collect_outside_fence`]);
/// the `&[&str]` signature is retained so the `lines()`-based call sites
/// stay unchanged (the join is fence-neutral: callers pass
/// already-normalized lines).
...
mod tests {
    ...
    // ========================================================================
    // P-3 differential corpus: pin the header-boundary scan semantics
    // (fence/indent/tilde/unclosed/nested-length/CRLF/empty) on the shared
    // scanner family (`header_indices` / `first_header_index` now delegate
    // to it; ported from the wip snapshot with the envelope wording of this
    // branch preserved).
    // ========================================================================
    ...
    // P-3: the centralized legacy-header twin must behave exactly like the
    // deleted ops-side original (identical pattern string, identical
    // matches).
    #[test]
