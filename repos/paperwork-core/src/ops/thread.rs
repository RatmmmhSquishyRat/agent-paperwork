...
pub fn thread_edit(path: &Path, seq: u64, sender: &str, new_body: &str) -> Result<()> {
    ...
    match incremental_offset {
        Some(offset) => {
            ...
            // re-serialized final message (per-step wording preserved via
            // the `file()` escape hatch, T4).
            ...
            handle
                ...
                .map_err(|e| PaperworkError::io_ctx(path, e, "check file handle validity", ""))?;
            handle
                .write_all(edited_serialized.as_bytes())
                .map_err(|e| {
                    PaperworkError::io_ctx(path, e, "check disk space and file permissions", "")
                })?;
        }
...
/// spec §5.5 mandates for SEQ resolution (and the send path already scans)
/// is reused — an implementation decision, not a spec mandate for the
/// sender lookup itself (Ultra Review F5 wording fix). Runs under an
...
pub fn find_message_sender(path: &Path, seq: u64) -> Result<Option<String>> {
    ...
    find_message_sender_locked(guard.file(), path, seq)
}
