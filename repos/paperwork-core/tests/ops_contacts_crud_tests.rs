...
use paperwork_core::ops::manifest::{
    brief_add_entry, brief_create, brief_read, brief_remove_entry,
};
...
fn contacts_remove_hit_preserves_title_and_order() {
    ...
    let path = setup_contacts(
        &dir,
        "Core Team",
        &["alice.profile.md", "bob.profile.md", "carol.profile.md"],
    );
...
fn contacts_update_hit_redrives_label_and_keeps_order() {
    ...
    assert_eq!(
        entries[0].label, "carol",
        "label must be re-derived from NEW profile H1 (R11)"
    );
...
fn contacts_update_label_fallback_to_stem_when_new_unreadable() {
    ...
    assert_eq!(
        entries[0].label, "ghost",
        "label must fall back to the file-name stem"
    );
}
...
fn contacts_update_old_equals_new_follows_judgment_order() {
    ...
    assert!(
        matches!(err, PaperworkError::AlreadyExists { .. }),
        "OLD==NEW with OLD hit must be AlreadyExists"
    );
    ...
    assert!(
        matches!(err, PaperworkError::NotFound { .. }),
        "OLD==NEW with OLD miss must be NotFound"
    );
}
...
fn remove_update_roundtrip_with_special_character_paths() {
    ...
    assert!(
        ...
        "space path must serialize in angle-bracket form"
    );
...
fn contacts_add_idempotent_after_lock_migration() {
    ...
    assert_eq!(
        std::fs::read(&path).unwrap(),
        before,
        "idempotent add must be a byte-level no-op"
    );
...
fn locked_writes_produce_bytes_identical_to_serialize_functions() {
    ...
    assert_eq!(
        content, expected,
        "contacts add output must equal serialize_contacts"
    );
    ...
    assert_eq!(
        content, expected,
        "contacts remove output must equal serialize_contacts"
    );
    ...
    assert_eq!(
        content, expected,
        "contacts update output must equal serialize_contacts"
    );
