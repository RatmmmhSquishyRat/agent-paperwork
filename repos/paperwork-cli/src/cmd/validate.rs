...
pub fn run(ctx: &Context, args: ValidateArgs) -> Result<()> {
    ...
    // Normalize once and hand the Cow down (T4): normalization is idempotent,
    // so the core parsers' internal re-normalize is a no-op borrow; this
    // removes the historical 3–4 repeated rewrites of the same content.
...
/// ok/error conclusion.
///
/// T4: the fence walk delegates to core's shared scanner
