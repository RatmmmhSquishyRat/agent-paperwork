...
//!
//! Backported from `wip/v0.5-perfection-snapshot-2026-08-15` commit
//! `5bfb061` via Plan-C selective backport (task #52); every call site is
//! rewritten to the v0.6 named grammar (docs/ssot/specs/cli-grammar-v0.6/)
//! and every pinned envelope re-pinned to master's current output.
//! See docs/dev/v05-wip-backport-2026-08-15.md for the per-scenario
//! disposition table.
//!
...
//!   envelope (default + JSON, both filters), summary on a missing file
//!   (master semantics: not-found exit 1 per the Kim M1 symmetric guard —
//!   the wip lenient empty-summary behavior is revoked), `--quiet/--plain`
//!   on error paths, CRLF input roundtrip, Unicode CLI roundtrip,
//!   injection-guard CLI regressions, and concurrent first-send contention
//!   (CONC-02 CLI face).
//!
...
/// envelope shape (error line + fix + example), complementing the JSON-face
/// substring assertion in cli_integration.rs (validate_rejects_legacy_contacts).
#[test]
...
/// status=error / category / command / message / fix / example / exit_code=1
/// keys, stderr empty, exit 1. Two representative bad examples (profile +
...
fn ivy_g3_validate_json_error_envelope_structure() {
    ...
    // master's JSON error objects carry the `command` key (v0.5 spec
    // frozen surface, kept by the v0.6 grammar).
    assert_eq!(
        r.stdout,
        "{\"category\":\"format\",\
         \"command\":\"validate\",\
    ...
    assert!(r.stdout.contains("\"command\":\"validate\""));
...
fn ivy_g4_edit_not_most_recent_cli_envelope_and_bytes_unchanged() {
    ...
    run(
        &dir,
        &["post", "send", "g4b", "--author", "bob", "--message", "b1"],
    );
    run(
        &dir,
        &[
...
fn ivy_g4_edit_not_final_cli_envelope_and_bytes_unchanged() {
    ...
    run(
        &dir,
        &["post", "send", "g4c", "--author", "bob", "--message", "b1"],
    );
...
/// the empty-result envelope shape (0 messages, showing 0/0, no window,
/// `[]` in JSON).
#[test]
fn ivy_g5_read_filters_no_match_empty_envelope() {
    ...
    run(
        &dir,
        &["post", "send", "f", "--author", "bob", "--message", "world"],
    );

    // Default mode: "0 messages" envelope with showing 0/0 and no window.
    ...
    // JSON mode: empty messages array, showing 0/0, status stays ok.
    let r = run(&dir, &["--json", "post", "read", "f", "--mention", "ghost"]);
    assert_eq!(r.code, 0);
    assert_eq!(
        r.stdout,
        "{\"command\":\"post.read\",\"conclusion\":\"0 messages\",\"messages\":[],\"showing\":\"0/0\",\"status\":\"ok\"}\n"
    );
}

/// summary on a missing thread — master semantics (Kim M1 symmetric guard):
/// the wip lenient empty-summary behavior is revoked; a missing thread is a
/// not-found exit 1 on the same shape as post read. Both faces pinned.
#[test]
fn ivy_g5_summary_missing_file_not_found_envelope() {
    let dir = TempDir::new().unwrap();

    let r = run(&dir, &["post", "summary", "ghost"]);
    assert_eq!(r.code, 1, "summary of a missing thread is not-found");
    assert!(r.stdout.is_empty());
    assert_eq!(
        r.stderr,
        "error not-found: Thread 'ghost.post.md' not found\n\
         fix: send a message first to create the thread\n\
         example: paperwork post send ghost.post.md --author alice --message \"Hello\"\n",
        "G5: missing-file summary envelope must stay byte-exact"
    );
    ...
    assert_eq!(
        r.stdout,
        "{\"category\":\"not-found\",\"command\":\"post.summary\",\
         \"example\":\"paperwork post send ghost.post.md --author alice --message \\\"Hello\\\"\",\
         \"exit_code\":1,\
         \"fix\":\"send a message first to create the thread\",\
         \"message\":\"Thread 'ghost.post.md' not found\",\"status\":\"error\"}\n"
    );
}

/// Error-path mode combinations: `--quiet` and `--plain` never alter the
/// stderr error envelope bytes, and `--quiet` never alters the `--json`
/// stdout error envelope bytes — exit code stays 1 everywhere.
#[test]
fn ivy_g5_error_envelope_quiet_plain_json_unchanged() {
    let dir = TempDir::new().unwrap();
    write(&dir, "legacy.post.md", "# Legacy\n\n### #1 alice (ts)\n");

    let default = run(&dir, &["validate", "legacy.post.md"]);
    let with_quiet = run(&dir, &["--quiet", "validate", "legacy.post.md"]);
    let with_plain = run(&dir, &["--plain", "validate", "legacy.post.md"]);

    assert_eq!(with_quiet.code, 1);
    assert_eq!(with_plain.code, 1);
    assert!(
        with_quiet.stdout.is_empty(),
        "quiet error path keeps stdout empty"
    );
    assert!(
        with_plain.stdout.is_empty(),
        "plain error path keeps stdout empty"
    );
    assert_eq!(
        with_quiet.stderr, default.stderr,
        "G5: --quiet must not alter the error envelope bytes"
    );
    assert_eq!(
        with_plain.stderr, default.stderr,
        "G5: --plain must not alter the error envelope bytes"
    );
    assert!(default.stderr.starts_with("error format:"));

    // JSON face: quiet must not alter the stdout error envelope either.
    let json = run(&dir, &["--json", "validate", "legacy.post.md"]);
    let json_quiet = run(&dir, &["--json", "--quiet", "validate", "legacy.post.md"]);
    assert_eq!(json_quiet.code, 1);
    assert!(json_quiet.stderr.is_empty());
    assert_eq!(
        json_quiet.stdout, json.stdout,
        "G5: --quiet must not alter the JSON error envelope bytes"
    );
    assert!(json.stdout.contains("\"status\":\"error\""));
}
